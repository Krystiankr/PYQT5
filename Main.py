...
from interface_files.Migrate import Ui_Migrate
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.ui.migrateBtn.clicked.connect(self.open_migrate_window)
        ...
        # migrate
        self.Migrate = QtWidgets.QDialog()
        self.migrate = Ui_Migrate()
        self.migrate.setupUi(self.Migrate)
        self.migrate.buttonBox.accepted.connect(self.json_words)
        ...
        # if you choose badly answer this variable increase 2 corr + 3 freq point
        self.tmp_score = 0
    ...
    def keyPressEvent(self, e):
        if e.key() == Qt.Key_1:
            print("Word_1")
            #self.polish_button_clicked(self.ui.cbutton1)
            self.ui.cbutton1.click()
        if e.key() == Qt.Key_2:
            print("Word_2")
            #self.polish_button_clicked(self.ui.cbutton2)
            self.ui.cbutton2.click()
        if e.key() == Qt.Key_3:
            print("Word_3")
            #self.polish_button_clicked(self.ui.cbutton3)
            self.ui.cbutton3.click()
        if e.key() == Qt.Key_R:
            print("R-Random")
            self.ui.random_button.click()
        if e.key() == Qt.Key_S:
            print("S-Save")
            self.ui.save_button.click()
        if e.key() == Qt.Key_P:
            print("P-Reset")
    ...
    def to_file(self):
        ...
        f.close()
        self.bar_change_text(today + " update log file ")
    ...
    def update_new_word(self):# ??need change this name
        if self.new_choice is not None:
            try:
                ...
                df.set_at(at_0=stare, at_1='Polski', what=self.new_choice[0].text())
    ...
    def polish_button_clicked(self, btn): # temporary stats
        ...
        if self.ui.random_word.text() in df[df['Polski'] == btn.text()]['Angielski'].values:
            ...
            if self.tmp_score == 0:
                check = self.df_data.increase_perfectScore(self.ui.random_word.text())
                if check == 2:
                    self.df_data.increase_frequency(self.ui.random_word.text(), number=3)
                    self.df_data.increase_perfectScore(self.ui.random_word.text(), reset=True)
            self.tmp_score = 0
        else:
    ...
    def save_button(self):
        try:
            ...
            print("ERROR")
            self.bar_change_text("Save ERROR!")
